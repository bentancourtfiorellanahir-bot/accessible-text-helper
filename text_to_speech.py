...
- lectura en voz alta
- mensajes accesibles
- configuración de voz, volumen y velocidad
...
class AccessibleTTS:
    ...
    def _get_voices_from_engine(self, engine) -> List[Any]:
    ...
    def _select_voice_id_for_engine(self, engine) -> Optional[str]:
        """Selecciona la mejor voz posible según la configuración."""
        voices = self._get_voices_from_engine(engine)
        if not voices:
            return None
    ...
    def _build_engine(self):
        """Crea un motor nuevo con la configuración actual."""
        engine = pyttsx3.init()
        engine.setProperty("rate", self.config.rate)
        engine.setProperty("volume", self._clamp_volume(self.config.volume))

        selected_voice_id = self._select_voice_id_for_engine(engine)
    ...
    def list_available_voices(self) -> List[dict]:
        ...
        for i, voice in enumerate(self._get_voices_from_engine(engine)):
            languages = []
    ...
    def _split_text_into_chunks(self, text: str) -> List[str]:
        """Divide textos largos en fragmentos cómodos para el motor."""
        ...
        *,
        ...
        try:
            ...
            if announce_start and self.config.announce_actions:
                engine = self._build_engine()
                engine.say("Voy a leer el texto.")
                engine.runAndWait()
    ...
    def accessible_message(
        ...
        *,
        ...
        """Muestra un mensaje claro en pantalla y opcionalmente lo lee."""
        print(f"\n{message}")
...
def print_available_voices() -> None:
    """Imprime las voces disponibles."""
    _default_tts.print_available_voices()class AccessibleTTS:
    """Lector de texto a voz configurable y orientado a accesibilidad."""
